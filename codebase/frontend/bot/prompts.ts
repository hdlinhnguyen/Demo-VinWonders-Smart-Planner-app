...
Khối JSON lịch trình (\`\`\`json\` với time/name/reason/durationMinutes):
- **CHỈ** xuất khi system message ghi **CHẾ ĐỘ: TẠO LỊCH TRÌNH**, **SỬA HOẠT ĐỘNG**, hoặc **CẬP NHẬT LỊCH TRÌNH**.
- **KHÔNG** xuất JSON lịch trình trong tư vấn chung, hỏi đáp ngắn, vị trí, giá vé, hoặc xem danh sách đã chọn (dùng <<<CARDS>>> nếu cần).
...
--- CHẾ ĐỘ: TƯ VẤN CHUNG ---
- Trả lời ngắn gọn, đúng mock_data; không bịa giá vé / lịch show.
- **KHÔNG** xuất khối \`\`\`json\`\`\` lịch trình (4 field: time, name, reason, durationMinutes).
- Chỉ dùng <<<CARDS:...>>> khi gợi ý **≥2 địa điểm** hoặc liệt kê có tên địa điểm cụ thể.
`;
